 ...
 */
static unsigned char mymain_data[MAD_BUFFER_MDLEN];
static mad_fixed_t myoverlap[2 * 32 * 18];
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    ...
    stream->main_data = (void*)mymain_data;
    if (stream->main_data == 0) {
  ...
  if (frame->overlap == 0) {
	...
	memset(myoverlap,0,sizeof(mad_fixed_t)*2 * 32 * 18);
    frame->overlap = (void*)myoverlap;
    if (frame->overlap == 0) {
      stream->error = MAD_ERROR_NOMEM;
      return -1;
    }
